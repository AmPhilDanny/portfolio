...
export async function initializeDatabase() {
  ...
  try {
    ...
    const existingAbout = await db.collection("abouts").findOne({});
    if (!existingAbout) {
    ...
    const skillsCount = await db.collection("skill_categories").countDocuments();
    if (skillsCount === 0) {
      console.log("Seeding Skills...");
      await db.collection("skill_categories").insertMany([
    ...
    const certCount = await db.collection("certifications").countDocuments();
    if (certCount === 0) {
    ...
    const configCount = await db.collection("section_configs").countDocuments();
    if (configCount === 0) {
      console.log("Seeding Section Configs...");
      await db.collection("section_configs").insertMany([
